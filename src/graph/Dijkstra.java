...
public class Dijkstra {
	...
	public void computeShortestDistances(boolean[] vVisited) {
		int nextVertex = 0;
		v = g.getVertices();
		ArrayList<Edge> inEdges = v.get(0).getInEdges();

		DistanceFromSource(inEdges, -1);  // Initialization

		// we visit every vertices
		for (int i = 0; i < v.size(); i++) {
			...
			for (int joinedEdge = 0; joinedEdge < currentVerticeEdges.size(); joinedEdge++) {
				...
				Vertex verticeEnStart = eNow.getStartVertex();
				Vertex verticeEnEnd = eNow.getEndVertex();

				int neighbourIndex;
				if (verticeEnStart.getId() == nextVertex) {
					neighbourIndex = nextVertex;
				}

				// only if not visited
				if (!(vVisited[neighbourIndex])) {
					int tentative = DistanceFromSource(currentVerticeEdges, nextVertex) + eNext.getWeight();

					if (tentative < DistanceFromSource(currentVerticeEdges, nextVertex)) {
						this.Distance.set(neighbourIndex, tentative);
					}
				}
			}

			// all neighbours checked so Vertice visited
			vVisited[nextVertex] = true;
